# MCP Relay - Process Persistence Analysis Request

## Question
The MCP Relay process does not stop when Claude Code is restarted. Can you identify why this occurs?

## Context
This is a Python-based MCP (Model Context Protocol) relay server that:
- Acts as a stdio-based MCP server for Claude Code
- Multiplexes connections to multiple WebSocket-based MCP backends
- Manages tool discovery and routing across multiple backend services
...
## Observed Behavior
When Claude Code is restarted, the MCP Relay process continues running and does not terminate. This means the relay persists across Claude Code restarts.

## Analysis Request
Please analyze the code and identify:
1. **Why does the relay process not stop when Claude Code restarts?**
2. What mechanism or lifecycle dependency allows this persistence?
3. Is this behavior intentional or a side effect of the implementation?
4. What specific code patterns enable this behavior?
